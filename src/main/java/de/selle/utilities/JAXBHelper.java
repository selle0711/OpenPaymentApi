...
public final class JAXBHelper {
	...
	/**
	 * Ist das Zeichen ein Satzzeichen
	 * 
	 * @param zeichen
	 * @return boolean
	 */
	public static boolean isPunctuation(char zeichen) {
		return "!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~".indexOf(zeichen) >= 0;
	}
	 ...
	 */
	public static XMLGregorianCalendar getXMLGregorianCalendar(final Date myDate)
			throws DatatypeConfigurationException {
		if (myDate == null) {
			return null;
		}
	 ...
	 * @throws SAXException
	 */
	public static <T> String marshalToXML(final Class<T> clazz, final T object, final String xsdFile)
			throws JAXBException, SAXException {
		return marshalToXML(clazz, object, xsdFile, false, null);
	}
	 ...
	 * @throws SAXException
	 */
	public static <T> String marshalToFormattedXML(final Class<T> clazz, final T object, final String xsdFile, final String noNamespaceName)
			throws JAXBException, SAXException {
		return marshalToXML(clazz, object, xsdFile, true, noNamespaceName);
	}

	private static <T> String marshalToXML(final Class<T> clazz, final T object, final String xsdFile,
			final boolean formattedOutput, final String noNamespaceName) throws JAXBException, SAXException {
		...
		if (noNamespaceName != null) {
			jaxbMarshaller.setProperty(Marshaller.JAXB_NO_NAMESPACE_SCHEMA_LOCATION,
					"C:\\\\Program Files\\\\Wincor Nixdorf\\\\Wincor Nixdorf OPI\\\\OPISchema\\\\"+noNamespaceName+".xsd");
		}
	 ...
	 * @throws SAXException
	 */
	public static <T> T unmarshallToJaxb(final Class<T> clazz, final String xml, final String xsdFile)
			throws JAXBException, SAXException {
		final SchemaFactory schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
		Schema xsdSchema = null;
		if (xsdFile != null) {
	 ...
	 * @throws SAXException
	 */
	public static <T> T unmarshallFileToJaxb(final Class<T> clazz, final File xmlFile, final String xsdFile)
			throws JAXBException, SAXException {
		final SchemaFactory schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
	 ...
	 * @throws SAXException
	 */
	public static <T> T unmarshallInputStreamToJaxb(final Class<T> clazz, final InputStream inputStream,
			final String xsdFile) throws JAXBException, SAXException {
